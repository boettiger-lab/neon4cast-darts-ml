...
def upload_df_to_s3(object_name, dataframe, s3_client):
...
def download_df_from_s3(object_name, s3_client):
    ...
    response = s3_client.get_object(Bucket='neon4cast-darts-ml', Key=object_name)
...
def ls_bucket(directory_prefix, s3_client):
    """
    List files in a directory (prefix) within an S3 bucket.
    """
    # List objects in the specified directory
    response = s3_client.list_objects_v2(Bucket='neon4cast-darts-ml', Prefix=directory_prefix)
