...
Over the last decade, machine learning has gained significant prominence after a spate of high profile successes based on neural network models, some of these successes include beating the master champion of Go, accurately predicting protein structures and the advent of large language models like ChatGPT \cite{alphago, alphafold, chatgpt}. During this time, researchers in computer science have developed new neural network models for time series forecasting with promises of significant accuracy improvements over previous methods \cite{makridakis}. Simultaneously during the last 10 years, there has been a growing focus in limnology towards using neural networks for water quality forecasting, but this focus has primarily centered on older neural network architectures and there has been little exploration of newer methods. This study aims to address this research gap by performing a comprehensive comparison of neural network models, including models taken from more recent research in computer science.

Instead of identifying that there is some new neural network model that performs exceptionally well, this study has affirmed the well-established forecasting axiom that ensemble forecasts are often more accurate and robust than forecasts from single models. We found that the Naive Ensemble model which aggregated the forecasts from the neural network models was the best performing model overall, evincing all the other models on the target variables of dissolved oxygen and water temperature, while also performing well for chlorophyll-a (attaining the second and third best CRPS and RMSE, respectively). This result affirms recent work from \cite{freya} that established multi-model ensembles as advantageous over individual models for water temperature forecasting.

Yet, as is often the case when judging whether one method is superior to another, we have found that subtle changes to our criteria may have produced radically different results. For instance, it is possible that if we changed the forecast horizon to 5 days, then the AutoTheta model may have been the best performing model as the AutoTheta model outperformed all the other models in short time horizons (see Figure \ref{}). But, it is important to qualify that changing our criteria to consider shorter term forecasts may result in the neural network models learning different behavior during training. How these models perform with different forecast horizons warrants future research.

The neural network models struggled to forecast Chlorophyll-A, a result that is not surprising given that forecasting algae blooms has been well established as a challenging prediction problem in limnology \cite{}. The neural network models displayed a tendency to take a conservative strategy, regularly predicting that the chlorophyll-a concentration would remain in a non-bloom state. It is possible that the neural network models settled on this conservative behavior as they were not provided enough information to predict blooms -- the neural networks were not provided with some of the typical covariates used in process-based chlorophyll-a models like Nitrogren and Phosphorous concentrations as well as measures of photosynthetic active radiation. And, it is likely that this conservative strategy is an artifact of the length of the forecast horizon. Blooms are stochastic events, characterized by rapid fluctuations in chlorophyll-a. Predicting the exact time of such an event in a long term horizon will be more difficult than predicting the onset of a bloom in the near-term \cite{}.

We hope that this comparative study will encourage more research that investigates neural networks for water quality forecasting. In place of the one research question that we attempted to answer, "How do neural network models perform for water quality forecasting?", many new questions and ideas have arose. Outside of exploring how the performance of neural networks will vary with forecast horizon, which was mentionned previously, we think that some other promising directions for exploration include cross-learning and hybrid models. 

Provided that there are large limnological datasets that currently exist, and there will be an increasing amount of limnological data coming online, neural network models offer significant advantages over process-based and statistical models as they will be able to learn directly from the data. This data-driven approach is advantageous in a few ways: as NN models can learn directly from the data, these models will not be limited by the assumptions inherent to process-based and statistical models. Moreover, these models should improve as 
