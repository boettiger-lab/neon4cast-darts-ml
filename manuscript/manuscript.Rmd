...
where $\nabla$ is the difference operator, $Z_t$ is defined as a theta line and $X_t$ is the original data. When $\theta < 1$, the second differences are reduced from the data, yielding a Theta line that amplifies the long term trends of the data. For instance, if $\theta = 0$, the theta line will be a linear regression of the data. And if $\theta > 1$, the data's short term behavior will be magnified in the Theta line. The Theta model generates a forecast by extrapolating the linear combination of two or more Theta lines. Although the Theta model is relatively simple, it has performed very well in the M3 and M4 forecasting competitions, which has motivated its use in this study as a benchmark statistical model \cite{}. In the time since the Theta model was originally presented in 2000, new variations of the Theta model have been presented which have been shown to outperform the original Theta model \cite{}. Throughout this study, we use the StatsForecast AutoTheta model which selects the best performing variation of the Theta model.
