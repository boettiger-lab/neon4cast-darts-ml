...
In Table \ref{}, we present the mean CRPS and RMSE scores for the respective target variables. For Dissolved Oxygen (DO) and Water Temperature (WT), there are similar results for model performance: the neural network models outperform the AutoTheta, Naive Persistence and Climatology model with few exceptions; and the naive ensemble model is the best performing model with respect to both CRPS and RMSE. Yet, with Chlorophyll-A, there are much different results: while the neural network models still generally outperform the reference models in CRPS, the Naive Ensemble model is no longer the top performing model in either CRPS or RMSE. BlockRNN attains the best performance with respect to CRPS and the Naive Persistence model attains the best mean RMSE.

Intuition for why we see the results in the scoring tables can be gained by looking at individual forecasts shown in Figure \ref{}. 
